...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (option === 'add' || option === 'remove' || option === 'set' || option === 'page') {
        ...
        if (option === 'example') {
            ...
            let page = 1;
            if (value != 'next' && value != ' n' && value != 'previous' && value != 'p') {
                if (/^\d+$/.test(value)) page = Number(value);
            } else {
                if (value === 'next' || value == 'n') page = user.song_page < steps.length ? user.song_page + 1 : user.song_page
                else if (value === 'previous' || value === 'p') page = user.song_page > 1 ? user.song_page - 1 : user.song_page
            }
            await Users.updateOne({ userId: interaction.user.id }, { $set: { song_page: page } });
            msg = await interaction.channel.messages.fetch(user.messageId);
            if (msg) msg.edit({ embeds: [await songManager('new', interaction)] });
        }
    }
}